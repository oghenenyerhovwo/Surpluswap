    ...
    useEffect(() => {
        if(transaction._id){
            const newForm = {
                ...
                popClient: (transaction.popClient && transaction.popClient.length > 1) || [{_id: 1, url: "www.freepik.com/bulaba"}],
            }
            setTransactionData(JSON.parse(localStorage.getItem(`surpluswap_non_active_transactions_${params.transactionId}`)) || {
                form: newForm,
                error: {paymentAccountNumber: "", paymentAccountName: "", popClient: {min: 1, text: ""},},
                page: 1,
            })
        }
    }, [dispatch, transaction, params.transactionId])
    ...
    useEffect(() => {
        ...
        if(transactionData.page === 3){
            setForm({
                ...
                paymentAccountName : (transactionData.form && transactionData.form.paymentAccountName) || "",
            })
            setError({
                paymentAccountNumber : (transactionData.error && transactionData.error.paymentAccountNumber) || "",
                paymentAccountName : (transactionData.error && transactionData.error.paymentAccountName) || "",
            })
        }
    ...
    const nextPage =e => {
        ...
        if(!isError){
                ...
                if(transactionData.page >= numberOfPages){
                    dispatch(updateTransaction({...transactionData.form, status: "pending-admin-approval", adminAccountDetails: accountDetailsNaira}, transaction._id))
                    setTransactionData({})
                } else {
                                    ...
                                    transactionData.page === 1 && (
                                        ...
                                        >
                                            <AccountDetailsPage 
                                                ...
                                                accountDetails={accountDetailsNaira}
                                            />
                                        </motion.div>
                                    )
                                }
