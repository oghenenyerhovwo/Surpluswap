...
const MyProfile = props => {
  ...
  const { 
    ...
    const transactionsCompleted = transactions
        .filter(transaction => transaction.status === "completed")
    const numberOfTransactionsCompleted = transactionsCompleted.length
    const transactionPending = transactions
        .filter(transaction => transaction.status === "pending")
    const transactionRejected = transactions
                ...
                </div>
                {
                    currentUser.lastName && (
                        <div className={`grid ${styles.about_section} spacing-sm`}>
                            ...
                            <h3 className={`soft_blue ${styles.about_section_label_text}`}>{currentUser.lastName}</h3>
                        </div>
                    )
                }
                <div className={`grid ${styles.about_section} spacing-sm`}>
                    <h3 className={styles.about_section_label}>Email:</h3>
                    <h3 className={styles.about_section_label_text}>{currentUser.email}</h3>
                </div>
                <div className={`grid ${styles.about_section} spacing-sm`}>
                    <h3 className={styles.about_section_label}>Phone Number:</h3>
                    <h3 className={styles.about_section_label_text}>{currentUser.phoneNumberTextWithCode}</h3>
                </div>
                <div className={`grid ${styles.about_section} spacing-sm`}>
